...
for runNumber in $(seq ${NUMBER_OF_RUNS}); do
    ant clean-all
    if ant generate-tests-jpf-MLB-whitebox ; then #&& fixRaytraceFloatZeros; then
	until ant compile-tests-jpf-MLB-whitebox; do
	    ant compile-tests-jpf-MLB-whitebox | disableNonCompilingTests
	done
	if ant run-tests-jpf-MLB-whitebox report-ex; then
