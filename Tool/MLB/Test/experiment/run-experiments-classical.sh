...
for runNumber in $(seq ${NUMBER_OF_RUNS}); do
    ant clean-all
    if ant generate-tests-jpf-MLB-classical ; then #&& fixRaytraceFloatZeros; then
	until ant compile-tests-jpf-MLB-classical; do
	    ant compile-tests-jpf-MLB-classical | disableNonCompilingTests
	done
	if ant run-tests-jpf-MLB-classical report-ex; then
