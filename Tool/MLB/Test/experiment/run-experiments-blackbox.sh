...
for runNumber in $(seq ${NUMBER_OF_RUNS}); do
    ant clean-all
    if ant generate-tests-jpf-MLB-blackbox ; then #&& fixRaytraceFloatZeros; then
	until ant compile-tests-jpf-MLB-blackbox; do
	    ant compile-tests-jpf-MLB-blackbox | disableNonCompilingTests
	done
	if ant run-tests-jpf-MLB-blackbox report-ex; then
