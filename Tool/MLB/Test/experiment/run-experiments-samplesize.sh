...
for runNumber in $(seq ${NUMBER_OF_RUNS}); do
    ant clean-all
    if ant generate-tests-jpf-MLB-samplesize ; then #&& fixRaytraceFloatZeros; then
	until ant compile-tests-jpf-MLB-samplesize; do
	    ant compile-tests-jpf-MLB-samplesize | disableNonCompilingTests
	done
	if ant run-tests-jpf-MLB-samplesize report-ex; then
