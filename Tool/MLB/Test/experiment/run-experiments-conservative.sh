...
for runNumber in $(seq ${NUMBER_OF_RUNS}); do
    ant clean-all
    if ant generate-tests-jpf-MLB-conservative ; then #&& fixRaytraceFloatZeros; then
	until ant compile-tests-jpf-MLB-conservative; do
	    ant compile-tests-jpf-MLB-conservative | disableNonCompilingTests
	done
	if ant run-tests-jpf-MLB-conservative report-ex; then
