...
for runNumber in $(seq ${NUMBER_OF_RUNS}); do
    ant clean-all
    if ant generate-tests-jpf-MLB-ECS ; then #&& fixRaytraceFloatZeros; then
	until ant compile-tests-jpf-MLB-ECS; do
	    ant compile-tests-jpf-MLB-ECS | disableNonCompilingTests
	done
	if ant run-tests-jpf-MLB-ECS report-ex; then
